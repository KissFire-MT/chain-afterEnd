 ...
 * @description 新增加通知
 */
 export const userAdd=asyncHandler(async (req: Request, res: Response, next: NextFunction)=>{
    const { sqlObj } = tools.sqlDeal(req.body);
    let result = null;
    if(true){
        const add = `insert into notice set ?`;
 ...
 */
 ...
 export const findchainList=asyncHandler(async (req: Request, res: Response, next: NextFunction)=>{
    let result = null;
    const decoded=req.decoded; //更具token的信息来查找
    const sqlStr = `select * from chain`;
    result = await mysqlDb.execute(sqlStr,  [decoded.id]);
 ...
 */
 ...
 export const findsystemList=asyncHandler(async (req: Request, res: Response, next: NextFunction)=>{
    let result = null;
    const decoded=req.decoded; //更具token的信息来查找
 ...
 */
 ...
 export const findNotice=asyncHandler(async (req: Request, res: Response, next: NextFunction)=>{
    let result = null;
    const decoded=req.decoded; //更具token的信息来查找
    const sqlStr = `select * from notice where isRead=0`;
    result = await mysqlDb.execute(sqlStr,  [decoded.id]);
    sendResponse(res, 200, sucessCallbackVal(code.successCode, result.data, '成功', true));
});
 ...
 * @description 权限检验
 */
 export const permissionCheck=asyncHandler(async (req: Request, res: Response, next: NextFunction)=>{
    const {username,uuid}=req.body;
    const sql = `select * from user where email=? and password=?`;
    const db=await mysqlDb.execute(sql,[username,uuid]);
    if(db.code===code.successCode){
        if(db.data.length){
            const {id,username} =db.data[0]; //获取重要的属性
            const createToken=author.createToken({id,username}); //生成token
            res.status(200).json(sucessCallbackVal(code.successCode,createToken,'成功',true));
        }else{
            res.status(200).json(sucessCallbackVal(code.failedCode,'uuid检验失败，或者无此用户','uuid检验失败，或者无此用户',false));
        }
    }else{
        res.status(500).json(sucessCallbackVal(code.failedCode,'服务器出错','失败',false));
    }
